#!/usr/bin/env python3
"""
Update all EN pages to use the new design from et/home.html.
Preserves each page's English <main> content, meta tags, structured data.
Replaces: fonts, CSS, header, footer, fixed elements, scripts.
English-specific: nav labels, footer text, booking button, cookie notice.
"""
import re
    ...
    <div class="container">
      <div class="footer-grid">
        <div>
          ...
          <div style="margin-top:6px;font-size:.82rem;">© 2025 MV Therapy – Massage in Pärnu</div>
        ...
        <div>
          ...
          <div>Rüütli 47, 2nd floor</div>
          <div>80010 Pärnu, Estonia</div>
          <a href="/en/map.html" style="font-size:.85rem;">📍 View on map</a>
        </div>
        <div>
        ...
        <div>
          <strong>Pages</strong>
          <div><a href="/en/services.html">Massage services</a></div>
          <div><a href="/en/Diagnosis.html">Diagnoses</a></div>
          <div><a href="/en/blog.html">Blog</a></div>
          <div><a href="/en/kkk.html">FAQ</a></div>
        </div>
      </div>
    </div>
  </footer>"""

# ─── Fixed elements (English) ─────────────────────────────────────────────────
FIXED_ELEMENTS = """
  <!-- GO TOP -->
  <button class="go-top" id="goTop" aria-label="Scroll to top of page">
  ...
  <div class="mobile-book-bar" id="mobileBookBar">
    ...
    <button class="btn btn-primary book-trigger">Book appointment</button>
  </div>

  <!-- COOKIE NOTICE -->
  <div id="cookieNotice" style="display:none;" role="dialog" aria-label="Cookie notice">
    <p>We use cookies to enhance your experience, personalize content and analyze traffic.</p>
    <button class="cookie-btn" id="cookieAccept">OK, accept</button>
  </div>"""

# ─── Scripts (English language for SalonLife) ─────────────────────────────────
NEW_SCRIPTS = """
  <!-- === SCRIPTS === -->
  ...
  <!-- SalonLife Booking Widget (English) -->
  <script>
    ...
    !function(e,n,t){
      ...
      window.SalonLifeWidget.mount({
        identifier:3986,button_text:book_texts.en,language:'en',color:'#44437d',container:'#broneeri-container'
      })
    },window);
  </script>
...
def get_active_section(rel_path):
    parts = rel_path.parts  # ('en', 'services.html') or ('en', 'services', '1.html')
    if len(parts) == 2:
        mapping = {
...
# ─── English nav items ─────────────────────────────────────────────────────────
NAV_ITEMS = [
    ('/en/home.html',      'Home',       'home'),
    ('/en/services.html',  'Massages',   'services'),
    ('/en/Diagnosis.html', 'Diagnoses',  'diagnosis'),
    ('/en/campaign.html',  'Campaigns',  'campaign'),
    ('/en/workshops.html', 'Training',   'workshops'),
    ('/en/team.html',      'Team',       'team'),
    ('/en/kkk.html',       'FAQ',        'kkk'),
    ('/en/blog.html',      'Blog',       'blog'),
]
...
def build_header(et_url, ru_url, en_url, active_section):
    ...
    return f"""  <a class="skip-link" href="#content">Skip to main content</a>

  <!-- ===== HEADER ===== -->
  <header class="site-header">
    <div class="container topbar">
      <a class="brand" href="/en/home.html" aria-label="MV Therapy home page">
      ...
      <nav class="desktop-nav" aria-label="Main menu">{desktop}
      </nav>
      <div class="header-actions">
        <div class="mobile-lang-header" aria-label="Language">
          <a href="{et_url}">EST</a>
          <a href="{ru_url}">RUS</a>
          <a href="{en_url}" class="active">ENG</a>
        </div>
        ...
        <button class="mobile-toggle" id="mobileToggle" aria-expanded="false" aria-controls="mobilePanel" aria-label="Open menu">
    ...
    <div class="mobile-panel" id="mobilePanel">
      <div class="container">
        <nav aria-label="Mobile menu">{mobile}
        </nav>
      </div>
    </div>
  </header>"""
...
def extract_main(html):
    ...
    if m:
        ...
        # Remove old broneeri-container if present
        content = re.sub(
...
def clean_schema(html):
    def _fix_json(m):
        ...
        # Fix English superlatives in name field
        content = re.sub(
            r'("name"\s*:\s*")((?:[^"]*(?:Best|#1|Top|Perfect|5\.0)[^"]*))(")',
            lambda x: x.group(1) + 'MV Therapy' + x.group(3), content
        )
        # Fix name fields still containing "MVTherapy" (old brand name)
        content = re.sub(
            r'("name"\s*:\s*")MVTherapy([^"]*")' ,
            lambda x: x.group(1) + 'MV Therapy' + x.group(2), content
        )
        # Fix alternateName arrays with superlatives
        content = re.sub(
            r'"alternateName"\s*:\s*\[([^\]]+)\]',
            lambda x: '"alternateName": ["MV Therapy"]'
            if any(w in x.group(1) for w in ['Best', '#1', 'Top', 'Perfect', 'Parim'])
            else x.group(0),
            content
        )
...
def process_file(filepath):
    ...
    ru_url = f'/ru/{rel_from_en}'
    en_url = f'/en/{rel_from_en}'
...
if __name__ == '__main__':
    en_dir = BASE / 'en'
    files = sorted(en_dir.rglob('*.html'))
